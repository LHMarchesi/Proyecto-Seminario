...
public class DragonBoss : BaseEnemy
{
    ...
    protected override void Update()
    {
        ...
        // El target no debe poder congelar la máquina de fases.
        // Si por cualquier motivo se perdió la referencia, intentamos recuperarla,
        // pero la progresión por vida sigue evaluándose igualmente.
        ResolveTargetIfNeeded();

        UpdateBossUI();

        // FASE 2 -> FASE 3:
        // La vida es la fuente de verdad. No dependemos de isAttacking,
        // transitioning ni de una referencia vieja de coroutine.
        if (currentPhase == 2 &&
        ...
        // Todo lo que sigue necesita jugador válido, pero no bloquea fases.
        if (target == null)
            return;
    ...
    private void ForceStartPhase3()
    {
        ...
        Log(
            "Threshold de Fase 3 alcanzado. Forzando transición limpia.");

        // Si quedó una referencia vieja por una coroutine interrumpida,
        // la descartamos antes de comenzar una transición nueva.
        if (phase3Routine != null)
        {
            StopCoroutine(phase3Routine);
            phase3Routine = null;
        }
